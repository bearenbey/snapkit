# shotcut

cross-platform (Qt), open-source (GPLv3) video editor - mltframework/shotcut

Packaged by snapkit from the release asset `shotcut-linux-x86_64-26.8.1.txz`,
tracked against [mltframework/shotcut](https://github.com/mltframework/shotcut).
This snap is not published or endorsed by the upstream project.

## Building

    snapkit build shotcut

`pack.py` checks the packed snap before it is handed back; plain
`snapcraft` skips those checks.

## Installing what you built

    sudo snap install --dangerous shotcut_26.8.1_amd64.snap

## Updating

`snapkit` checks that upstream for a newer release and rewrites
`snap/snapcraft.yaml` for you. Anything you change in that file is kept:
an update only moves the version, the source URL and its checksum.
